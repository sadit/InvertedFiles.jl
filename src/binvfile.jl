...
Base.length(idx::BinaryInvertedFile) = length(idx.sizes)
...
Base.show(io::IO, idx::BinaryInvertedFile) = print(io, "{$(typeof(idx)) vocsize=$(length(idx.lists)), n=$(length(idx))}")
...
function Base.push!(idx::BinaryInvertedFile, obj; pools=nothing)
    push!(idx.sizes, length(obj))
    n = length(idx)

    @inbounds for tokenID in obj
        push!(idx.lists[tokenID], n)
    end
